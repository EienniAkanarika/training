...
url = ''  # 根据具体情况设置IP与端口
scan_speed = 'sequential'  # 设置扫描速度，由慢到快:sequential slow moderate fast
'''
爬虫模式配置
...
def stop_target(target_id):  # 停止扫描
    ...
    respon = requests.get(url=url_5, headers=headers, verify=False).content.decode('utf-8')
    print(respon)
...
def get_target_result(target_id, scan_session_id):  # 查询扫描结果,存在延迟
...
def main():
    for i in open('url.txt'):
        ...
        start_target(target_id, '11111111-1111-1111-1111-111111111111')  # 可行设置扫描类型
        Num = check_target_status()
        while int(Num) > 3:  # 设置最多同时进行的扫描数量，避免运行过多，导致服务器卡顿
            time.sleep(30)
            Num = check_target_status()
            print("当前扫描数量过多，正在等待添加！")
        time.sleep(10)  # 延时一会，避免Num获取不准
